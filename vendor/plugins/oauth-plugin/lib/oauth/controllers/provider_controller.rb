    ...
    module ProviderController
      def self.included(controller)
        controller.class_eval do
          before_filter :ensure_logged_in, :only => [:authorize,:revoke]
          oauthenticate :only => [:test_request]
          oauthenticate :strategies => :token, :interactive => true, :only => [:invalidate,:capabilities]
          oauthenticate :strategies => :two_legged, :interactive => true, :only => [:request_token]
          oauthenticate :strategies => :oauth10_request_token, :interactive => true, :only => [:access_token]
          skip_before_filter :verify_authenticity_token, :only=>[:request_token, :access_token, :invalidate, :test_request, :token]
        end
      end

      def request_token
        @client_application = ClientApplication.find_by_key! params[:oauth_consumer_key]

        # @token = current_client_application.create_request_token params
        @token = @client_application.create_request_token params
        if @token
      ...
      def authorize
        if params[:oauth_token]
          ...
          if request.post?
            @authorizer = OAuth::Provider::Authorizer.new :user, user_authorizes_token?, params
